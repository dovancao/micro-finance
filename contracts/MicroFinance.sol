...
contract MicroFinance {
    ...
    function createLoan(uint256 _amount) public onlyOutOfBlackList(msg.sender) onlyValidLoan(_amount)  returns(bool success){
        ...
        microFinaces[totalLoan.add(1)] = newLoan;
    ...
    function sendLoanToReceiver(address _receiver, uint256 _amount) public constant onlyOutOfBlackList(_receiver) onlyValidLoan(_amount) returns(bool success){
    ...
    function sendLoanInDeadline(address _borrower) public onlyCreatorOfContract(_borrower) returns (bool success) {
    ...
    /// @param _borrower address of required comfirmations
    function sendLoanBeforeDeadline(address _borrower) public onlyCreatorOfContract(_borrower) returns(bool success){
        require(now.sub(microFinaces[_borrower].start) < 3 days);
    ...
    /// @param _borrower address of required comfirmations
    function failed(address _borrower) public onlyCreatorOfContract(_borrower) returns (bool success){
        require(now.sub(microFinaces[_borrower].start) < 3 days);
